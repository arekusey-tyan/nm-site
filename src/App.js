...
import {styles} from './style-project.js';
...
function App() {
	if (action === 'work') {
		...
		if (['recipes'].includes(subaction)) {
			return (
				<>
					<HeadProvider>
						<Title>Рецепты | Рабочая зона | NanoMagic RPG | LoliLand.ru</Title>
					</HeadProvider>
					<WorkPage />
				</>
			)
	...
	if (action === 'lostpassword') {
		return (
			<>
				<HeadProvider>
					<Title>Lost Password | NanoMagic RPG | LoliLand.ru</Title>
				</HeadProvider>
				<LostPassword />
			</>
		)
	...
	if (action === 'register') {
		return (
			<>
				<HeadProvider>
					<Title>Registration | NanoMagic RPG | LoliLand.ru</Title>
				</HeadProvider>
				<RegisterPage />
			</>
		)
