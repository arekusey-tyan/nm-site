...
import {
	inputs, version,
	allRemoveRecipe, showResult3x3less,
	changeVersion, setInputs,
	showPopupO, clearContent, setMode,
	showPopupN, searchItem, submitN, renderTabs, saveRecipe
} from './functions.js'
import {ItemIngredient} from './ItemIngredient.js'

function RecipeWorkBench3x3Shapeless() {
	setMode('shapeless3x3')
		...
		<>
			...
			<div className={'tabs'} style={{position: 'absolute', top: '50px', right: '20px'}}>
				...
				<section style={{position: 'relative', top: '0px', left: '10px'}}>
					<button type={'submit'} className={'cybr-btn'} style={{padding: '15px', margin: 0}} data-type={'shapeless'} onClick={saveRecipe}><span /><span /><span /><span />Сохранить</button>
					<button type={'submit'} className={'cybr-btn'} style={{padding: '15px', margin: 0}} onClick={showResult3x3less}><span /><span /><span /><span />Результат</button>
					<div className={''}>
...
export default RecipeWorkBench3x3Shapeless