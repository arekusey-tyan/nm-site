...
import {logIn, user_info, tasks} from '../utils/Request.js'
						...
						<a href="#" role="button" style={{ float: 'left' }} onClick={event => event.preventDefault()}>
							<table>
								...
								<tr>
									...
									<td><div className="logo" id="navbarDropdownMenuLink" /></td>
								</tr>
								</tbody>
							</table>
						</a>
						...
						<div className="nav-item dropdown drp active">
							<a className="nav-link p-2 p-md-4" href="#" id="login_menu" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="true">
								<div className="d-flex align-items-center">
									<img width={18 + 'px'} height={18 + 'px'} src={user_info.foto} className="profoto mr-2" alt={''} />
									{user_info.name}
								</div>
							</a>
							<div className='dropdown-menu dropdown-menu-right' aria-labelledby='login_menu'>
								{user_info.user_group < 6 ? <a className='dropdown-item dfx' href='/addnews'>
									<div className="ecn">
								...
								<a className='dropdown-item dfx' href={`/user/${user_info.name}`}>
									<div className="ecn">
								...
								{user_info.user_group != 12 ? <a className='dropdown-item dfx' href={`/work/task-in-work`}>
									<div className="ecn">
										<i className="fa fa-bolt" aria-hidden="true" />
									</div>
									<div className="mr-4">Задачи</div>
									<div style={{ color: '#FFA500', textDecoration: 'none' }} className="ml-auto f80">{tasks.count}</div>
								</a> : ''}
								<div className="dropdown-divider" />
								{user_info.user_group < 4 ? <a className="dropdown-item" target="_blank" href="/admin">Управление сайтом</a> : ''}
								<a className="dropdown-item" href='/api/logout' onClick={logOut}>Выйти</a>
							</div>
						</div>
					</div>
					<div style={{ float: 'right' }}>
